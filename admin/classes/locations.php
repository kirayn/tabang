...
class locations extends DbTable
{
	...
	public function __construct()
	{
		...
		// locationId
		$this->locationId = new DbField('locations', 'locations', 'x_locationId', 'locationId', '`locationId`', '`locationId`', 21, -1, FALSE, '`locationId`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->locationId->IsAutoIncrement = TRUE; // Autoincrement field
		$this->locationId->IsPrimaryKey = TRUE; // Primary key field
		...
		// title
		$this->title = new DbField('locations', 'locations', 'x_title', 'title', '`title`', '`title`', 200, -1, FALSE, '`title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// pincode
		$this->pincode = new DbField('locations', 'locations', 'x_pincode', 'pincode', '`pincode`', '`pincode`', 19, -1, FALSE, '`pincode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getDetailUrl()
	{
		...
		if ($this->getCurrentDetailTable() == "adverts") {
			...
			$detailUrl .= "&fk_locationId=" . urlencode($this->locationId->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "locationslist.php";
		return $detailUrl;
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$rs['locationId'] = $this->locationId->DbValue;
		}
		return $success;
	}
	...
	public function update(&$rs, $where = "", $rsold = NULL, $curfilter = TRUE)
	{
		...
		$cascadeUpdate = FALSE;
		$rscascade = array();
		if ($rsold && (isset($rs['locationId']) && $rsold['locationId'] <> $rs['locationId'])) { // Update detail field 'locationId'
			$cascadeUpdate = TRUE;
			$rscascade['locationId'] = $rs['locationId']; 
		}
		if ($cascadeUpdate) {
			if (!isset($GLOBALS["adverts"]))
				$GLOBALS["adverts"] = new adverts();
			$rswrk = $GLOBALS["adverts"]->loadRs("`locationId` = " . QuotedValue($rsold['locationId'], DATATYPE_NUMBER, 'DB')); 
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('locationId', $rs))
				AddFilter($where, QuotedName('locationId', $this->Dbid) . '=' . QuotedValue($rs['locationId'], $this->locationId->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter <> "")
	...
	public function delete(&$rs, $where = "", $curfilter = FALSE)
	{
		...
		if (!isset($GLOBALS["adverts"]))
			$GLOBALS["adverts"] = new adverts();
		$rscascade = $GLOBALS["adverts"]->loadRs("`locationId` = " . QuotedValue($rs['locationId'], DATATYPE_NUMBER, "DB")); 
	...
	public function getRecordFilter($row = NULL)
	{
		...
		$val = is_array($row) ? (array_key_exists('locationId', $row) ? $row['locationId'] : NULL) : $this->locationId->CurrentValue;
		if (!is_numeric($val))
			return "0=1"; // Invalid key
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@locationId@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "locationsedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "locationsadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("locationsview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("locationsview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "locationsadd.php?" . $this->getUrlParm($parm);
		else
			$url = "locationsadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("locationsedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("locationsedit.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("locationsadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("locationsadd.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("locationsdelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "locationId:" . JsonEncode($this->locationId->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm <> "")
			$url .= $parm . "&";
		if ($this->locationId->CurrentValue != NULL) {
			$url .= "locationId=" . urlencode($this->locationId->CurrentValue);
		} else {
	...
	public function getFilterFromRecordKeys()
	{
		...
		foreach ($arKeys as $key) {
			...
			$this->locationId->CurrentValue = $key;
