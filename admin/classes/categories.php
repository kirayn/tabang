...
class categories extends DbTable
{
	...
	public function __construct()
	{
		...
		// categoryId
		$this->categoryId = new DbField('categories', 'categories', 'x_categoryId', 'categoryId', '`categoryId`', '`categoryId`', 20, -1, FALSE, '`categoryId`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// name
		$this->name = new DbField('categories', 'categories', 'x_name', 'name', '`name`', '`name`', 200, -1, FALSE, '`name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// description
		$this->description = new DbField('categories', 'categories', 'x_description', 'description', '`description`', '`description`', 201, -1, FALSE, '`description`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		...
		// parentId
		$this->parentId = new DbField('categories', 'categories', 'x_parentId', 'parentId', '`parentId`', '`parentId`', 20, -1, FALSE, '`parentId`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->parentId->Nullable = FALSE; // NOT NULL field
		$this->parentId->Required = TRUE; // Required field
		$this->parentId->Sortable = TRUE; // Allow sort
		$this->parentId->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->parentId->PleaseSelectText = $Language->phrase("PleaseSelect"); // PleaseSelect text
		switch ($CurrentLanguage) {
			case "en":
				$this->parentId->Lookup = new Lookup('parentId', 'categories', FALSE, 'categoryId', ["name","","",""], [], [], [], [], [], [], '', '');
				break;
			default:
				$this->parentId->Lookup = new Lookup('parentId', 'categories', FALSE, 'categoryId', ["name","","",""], [], [], [], [], [], [], '', '');
				break;
		...
		// image
		$this->image = new DbField('categories', 'categories', 'x_image', 'image', '`image`', '`image`', 201, -1, TRUE, '`image`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
	...
	public function getDetailUrl()
	{
		...
		if ($this->getCurrentDetailTable() == "adverts") {
			...
			$detailUrl .= "&fk_categoryId=" . urlencode($this->categoryId->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "categorieslist.php";
		return $detailUrl;
	}
	...
	public function update(&$rs, $where = "", $rsold = NULL, $curfilter = TRUE)
	{
		...
		if ($cascadeUpdate) {
			...
			$rswrk = $GLOBALS["adverts"]->loadRs("`categoryId` = " . QuotedValue($rsold['categoryId'], DATATYPE_NUMBER, 'DB')); 
	...
	public function delete(&$rs, $where = "", $curfilter = FALSE)
	{
		...
		if (!isset($GLOBALS["adverts"]))
			$GLOBALS["adverts"] = new adverts();
		$rscascade = $GLOBALS["adverts"]->loadRs("`categoryId` = " . QuotedValue($rs['categoryId'], DATATYPE_NUMBER, "DB")); 
	...
	public function getRecordFilter($row = NULL)
	{
		...
		$val = is_array($row) ? (array_key_exists('categoryId', $row) ? $row['categoryId'] : NULL) : $this->categoryId->CurrentValue;
		if (!is_numeric($val))
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("categoriesview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("categoriesview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "categoriesadd.php?" . $this->getUrlParm($parm);
		else
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("categoriesedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("categoriesedit.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("categoriesadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("categoriesadd.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("categoriesdelete.php", $this->getUrlParm());
	}
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($this->categoryId->CurrentValue != NULL) {
			$url .= "categoryId=" . urlencode($this->categoryId->CurrentValue);
		} else {
